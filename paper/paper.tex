...
global: PASS) while iso-shuffle and target-shuffle incremental nulls are
near zero ($\Delta R^2 < 0.001$), ruling out density artefacts.
...
\textit{SE}\cdot 8$.
Class 15 (all four diagonal neighbours alive) never occurs in practice
for an embedded isolate that is alive at $t$ (all orthogonal neighbours
are dead, so the GoL/HighLife birth rule does not make isolated cells
with 4 alive diagonal neighbours common in random initial states).
