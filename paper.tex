...
One candidate account is a \textit{cascade effect}: an embedded cell may
temporarily stabilise adjacent configurations, so its death triggers
secondary structural failures that propagate across multiple components
beyond the local $5\times5$ patch.
Under this account, the effective causal reach of an embedded cell's death
extends further than the local null can capture, which would produce the
observed negative excess.
A direct test would track downstream component changes triggered by each
embedded cell's removal across a larger causal window; such a localized causal
audit is a natural next step.
