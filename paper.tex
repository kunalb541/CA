...
A stable empirical relation linking embedded isolated cells to future
component-level decline holds across six size-by-density conditions
within GoL (mean slope $\approx-1.52$, CV~$\approx0.09$; two grid sizes,
three density bands, one deterministic rule).
...
Component-change law   & $64^2/128^2$   & 300--400 & 250 & 20260328+$k$, $k=0,\ldots,5$ & OLS slope \\
\bottomrule
\end{tabular}
\end{table}
...
$B=8$ with CI $[8,8]$.
The $[8,8]$ interval is the paper's sharpest single result: across all
1000 bootstrap resamples of the world distribution, not one resample placed
the peak anywhere other than $B=8$.
The block observer's own native scale is maximally informative about its own
future population under every perturbation of the world sample examined.

\section{A Stable Empirical Relation for Component Change}
\label{sec:D}
...
This reflects variation in outcome noise across density regimes, not
instability in the slope.
In low-density conditions the post-transient grid converges toward sparse
stable structures with relatively constrained component trajectories,
reducing noise and raising $R^2$ (peak: $R^2=0.684$, $128^2$ low-density).
In high-density conditions the dynamics are richer and the outcome variance
is higher, suppressing $R^2$ even though the slope is comparable.
The predictor slope captures the signal-to-signal ratio across conditions;
$R^2$ captures signal-to-noise within a condition and is expected to vary
as the noise floor changes.
...
The CI lies entirely below zero: the indirect effect is not merely small but
reliably negative, indicating a suppression pattern.
The a-path coefficient (embedded $\to$ $\fnet$) is $-1.225$: more embedded
cells predict more negative $\fnet$.
The b-path coefficient ($\fnet$ $\to$ $G$) is $+3.76\times10^{-4}$:
more negative $\fnet$ corresponds to slightly more positive $G$, as expected
(lower fine-scale component count reduces $\netF$, widening the gap).
The product $a\times b$ is therefore negative: via the $\fnet$ pathway,
embedded cells predict a reduction in $G$, opposing the positive direct
association.
This suppression arises because the b-path is extremely weak (near zero),
so the indirect pathway carries negligible positive signal while the
product of the two slopes yields a small but reliably negative value.
The direct effect ($+6.83\times10^{-3}$) exceeds the total effect
($+6.37\times10^{-3}$) because the suppressor removes noise, not signal.
The conclusion is unambiguous: fine-scale component net change over $[50,250]$
is not a positive mediator between embedded cell count and the narrative gap.
...
A fully aligned test would use $G_{[50,250]}$ as the outcome; given that the
indirect effect is already reliably negative under the current design, an
aligned version would if anything strengthen the null result.
...
\paragraph{Relation to causal emergence frameworks.}
Causal emergence in the sense of \citet{hoel2013} is defined via effective
information (EI): a macro-scale description exhibits causal emergence if
intervening at the macro level reduces uncertainty about effects more than
intervening at the micro level, measured by the KL divergence of the
intervention-conditional distribution.
The narrative gap $G$ used here is categorically different.
$G$ is a signed scalar — the difference between two observers' normalised
net-change counts over the same window — and carries no information-theoretic
interpretation: it can be positive or negative, is not invariant to invertible
reparametrisation, and measures signed observer disagreement rather than
information loss or causal leverage.
GoL is the test bed because it provides deterministic, reproducible,
multi-scale dynamics with analytically characterisable structure; the
specific questions asked (predictability of disagreement, scale-dependence
of forecasting optimum, coefficient stability) are designed to have
binary verdicts that do not depend on information-theoretic choices.
The keyword ``causal emergence'' in this paper locates the work in the
broader observer-dependence-of-description literature; it does not claim
any specific relationship to EI-based emergence measures, and the results
do not transfer to EI comparisons without substantial additional analysis.
...
Three empirical regularities in Conway's Game of Life are robustly
established under fixed, pre-specified observer definitions and decision
thresholds.
Observer disagreement is strongly predictable from early fine-scale dynamics
(partial $r=-0.848$, 95\% bootstrap CI $[-0.862,-0.830]$).
The optimal predictive scale is target-relative: live-cell count peaks at
$B=2$ and occupied-block count peaks at $B=8$, with non-overlapping
bootstrap confidence intervals.
Embedded isolated cells predict future component-level decline with a stable
slope across six size-by-density conditions within GoL
(mean $\approx-1.52$, CV~$=0.087$; two grid sizes, three density bands).

Two stronger mechanism interpretations were pre-specified and explicitly
rejected: global mediation through component net change fails (mediation
fraction $-0.072$, CI entirely below zero), and no positive residual
coupling survives the dynamic local null ($\chi<0$ in all six conditions).
Follow-up audits further show the slope does not reduce to a per-cell causal
rate under any tested local account.
The leading open question is whether the count of embedded isolated cells
functions as a compact marker of world-level structural fragility rather than
as a direct causal mechanism.
All results are scoped to this system; generalisation to other Life-like rules
and other physical systems remains an open empirical question.

\begin{acknowledgments}
No external funding was received for this work.
Simulations were performed using Python~3 with NumPy, SciPy, scikit-learn,
...
\textbf{K.B.:} conceptualization, methodology, software, investigation,
